...
%==============================================================================
% GARDENER INTERFACE FLOW DIAGRAM (Design Team Request)
%==============================================================================
\subsection{Gardener Interface Flow}

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.7, transform shape,
    node distance=1.5cm,
    component/.style={rectangle, rounded corners, draw, minimum width=2.5cm, minimum height=0.9cm, align=center, font=\small},
    human/.style={ellipse, draw, minimum width=2cm, minimum height=1cm, align=center, font=\small},
    arrow/.style={-{Stealth}, thick},
    bidiarrow/.style={{Stealth}-{Stealth}, thick}
]

% Human
\node[human, fill=green!20] (gardener) at (0,0) {Gardener};

% Interface components
\node[component, fill=blue!20] (hsm) at (4,0) {HSM\\Token};
\node[component, fill=yellow!20] (auth) at (8,0) {Auth\\Layer};
\node[component, fill=orange!20] (interface) at (12,0) {Interface\\Gateway};

% Backend
\node[component, fill=purple!20] (telemetry) at (12,2.5) {Telemetry\\(App H)};
\node[component, fill=red!20] (escalation) at (12,-2.5) {Escalation\\(App F)};
\node[component, fill=gray!20] (audit) at (16,0) {Audit\\Log};

% Arrows
\draw[arrow] (gardener) -- node[above, font=\tiny] {biometric} (hsm);
\draw[arrow] (hsm) -- node[above, font=\tiny] {challenge} (auth);
\draw[arrow] (auth) -- node[above, font=\tiny] {session} (interface);
\draw[arrow] (telemetry) -- (interface);
\draw[arrow] (interface) -- (escalation);
\draw[arrow] (interface) -- (audit);

% Bidirectional
\draw[bidiarrow] (interface) -- node[left, font=\tiny] {observe} (telemetry);
\draw[bidiarrow] (interface) -- node[left, font=\tiny] {override} (escalation);

\end{tikzpicture}
\caption{Gardener interface authentication and data flow.}
\label{fig:gardener-flow}
\end{figure}

\begin{table}[H]
\centering
\caption{Gardener interface actions and permissions.}
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Action} & \textbf{Permission} & \textbf{Auth Level} & \textbf{Audit} \\
\midrule
...
Emergency Halt (swarm) & EMERGENCY\_HALT & HSM + biometric + 2nd Gardener & Write + justification \\
...
\begin{notebox}
\textbf{Latency Requirements:} Observation flows must complete within 100ms for real-time monitoring. Override commands must acknowledge within 500ms. Emergency Halt must propagate to all affected capsules within 10ms (Reflex-tier).
\end{notebox}
...
%==============================================================================
% GARDENER INTERACTION FLOW (Report §4)
%==============================================================================
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.65, transform shape,
    node distance=0.8cm,
    box/.style={rectangle, rounded corners, draw, minimum width=1.8cm, minimum height=0.5cm, align=center, font=\tiny},
    decision/.style={diamond, draw, minimum width=1cm, minimum height=0.6cm, align=center, font=\tiny, aspect=2},
    arrow/.style={-{Stealth}, thick}
]

% Timeline
\draw[thick, gray] (0,0) -- (16,0);
\node[font=\tiny, gray] at (0,-0.3) {Event};
\node[font=\tiny, gray] at (16,-0.3) {Resolution};

% Alert arrives
\node[box, fill=red!20] (alert) at (1,1.5) {Alert\\Arrives};

% Gardener observes
\node[box, fill=blue!20] (observe) at (3.5,1.5) {Gardener\\Observes};

% Decision
\node[decision, fill=yellow!20] (decide) at (6,1.5) {Action\\Needed?};

% No action path
\node[box, fill=gray!20] (dismiss) at (6,3) {Dismiss\\+ Log};

% Override path
\node[box, fill=orange!20] (auth) at (8.5,1.5) {HSM\\Auth};

% Override type
\node[decision, fill=yellow!20] (type) at (11,1.5) {Override\\Type?};

% Actions
\node[box, fill=red!20] (halt) at (11,3.5) {HALT};
\node[box, fill=orange!20] (quarantine) at (13,3) {QUAR-\\ANTINE};
\node[box, fill=green!20] (fork) at (13,1.5) {FORCE\\FORK};
\node[box, fill=purple!20] (purge) at (13,0) {PURGE};

% Log
\node[box, fill=blue!20] (log) at (15,1.5) {ZK-SP\\Logged};

% Arrows
\draw[arrow] (alert) -- (observe);
\draw[arrow] (observe) -- (decide);
\draw[arrow] (decide) -- node[left, font=\tiny] {No} (dismiss);
\draw[arrow] (decide) -- node[above, font=\tiny] {Yes} (auth);
\draw[arrow] (auth) -- (type);
\draw[arrow] (type) -- (halt);
\draw[arrow] (type) -- (quarantine);
\draw[arrow] (type) -- (fork);
\draw[arrow] (type) -- (purge);
\draw[arrow] (halt) -| (log);
\draw[arrow] (quarantine) -| (log);
\draw[arrow] (fork) -- (log);
\draw[arrow] (purge) -| (log);

\end{tikzpicture}
\caption{Gardener interaction flow: From alert to resolution with ZK-SP logging.}
\label{fig:gardener-flow}
\end{figure}
