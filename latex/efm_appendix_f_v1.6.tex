...
\begin{table}[H]
\centering
\caption{Severity (S) to Escalation Level (L) mapping.}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Severity} & \textbf{Description} & \textbf{Escalation} & \textbf{Authority} \\
\midrule
...
\begin{notebox}
\textbf{Terminology Note:} Severity (S0--S3) classifies the \textit{urgency} of an event. Escalation Level (L1--L5) specifies the \textit{authority scope} required to respond. High severity events escalate to higher authority levels. See Canonical Terminology for definitions.
\end{notebox}
...
The following examples demonstrate complete escalation workflows for common scenarios:

%------------------------------------------------------------------------------
\subsection{Example 1: Dialect Drift Escalation}
%------------------------------------------------------------------------------

\begin{scenariobox}[Dialect Drift Detection and Resolution]
\textbf{Scenario:} A capsule cluster in Trunk-A7 begins using semantic patterns that diverge from the canonical LIG.

\vspace{0.2cm}
\textbf{Initial Conditions:}
\begin{itemize}
    \item Trunk-A7: 42 capsules, $SCI = 0.82$, $DDI = 0.08$
    \item DDI threshold: $\theta_{drift} = 0.15$
    \item Capsule C-A7-017 begins emitting non-standard sememes
\end{itemize}

\vspace{0.2cm}
\textbf{Phase 1: Detection (L1)}
\begin{enumerate}
    \item Telemetry (Appendix H) detects: $DDI$ rising 0.08 → 0.11 over 500 ticks
    \item Reflex logs semantic anomaly tag \texttt{FSS:DELTA} for C-A7-017
    \item Local monitoring continues; no escalation yet
\end{enumerate}

\vspace{0.2cm}
\textbf{Phase 2: Pattern Spread (L2)}
\begin{enumerate}
    \setcounter{enumi}{3}
    \item 3 additional capsules adopt C-A7-017's semantic patterns
    \item $DDI$ crosses 0.15 → Level 2 escalation triggered
    \item DEL (Appendix D) quarantines cross-trunk messages from affected capsules
    \item Local Arbiter convenes d-CAM quorum to assess
\end{enumerate}

\vspace{0.2cm}
\textbf{Phase 3: Arbiter Deliberation (L2--L3)}
\begin{enumerate}
    \setcounter{enumi}{7}
    \item Arbiter classifies: ``Dialect Innovation'' (not threat)
    \item ATP projection: 60\% probability of trunk fork within 2000 ticks
    \item Decision: Probation for 4 capsules; monitor for enshrinement potential
    \item Gardener notified (info only); no override required
\end{enumerate}

\vspace{0.2cm}
\textbf{Outcome:} After 1500 ticks, dialect stabilizes. Judicial Swarm (Appendix L) reviews and authorizes controlled fork. New Trunk-A7b created with documented semantic variant.
\end{scenariobox}

%------------------------------------------------------------------------------
\subsection{Example 2: Swarm Arbitration Conflict}
%------------------------------------------------------------------------------

\begin{scenariobox}[Cross-Trunk Arbitration Dispute]
\textbf{Scenario:} Two trunks disagree on capsule ownership after a merge operation.

\vspace{0.2cm}
\textbf{Dispute:}
\begin{itemize}
    \item Trunk-X12 claims capsule C-001 based on lineage
    \item Trunk-Y34 claims C-001 based on execution context
    \item Local Arbiters deadlock (3--3 vote)
\end{itemize}

\vspace{0.2cm}
\textbf{Escalation Path:}
\begin{enumerate}
    \item Deadlock detected → Level 3 (Auditor involvement)
    \item Auditor A-099 examines ZK-SP lineage proofs for C-001
    \item Lineage shows: C-001 spawned in X12, migrated to Y34 via approved transfer
    \item Auditor ruling: Y34 has custody; X12 retains lineage attribution rights
\end{enumerate}
...
\begin{enumerate}
    \setcounter{enumi}{4}
    \item X12 Arbiter appeals to Judicial Swarm (Appendix L)
    \item Judicial Swarm convenes: 7 Judicial Capsules from neutral trunks
    \item Review confirms Auditor ruling; precedent logged to d-CTM
    \item Case closed at Level 3; no Gardener involvement needed
\end{enumerate}

\vspace{0.2cm}
\textbf{Outcome:} Dispute resolved algorithmically. New precedent: ``Migration transfers custody; spawning trunk retains lineage attribution.''
\end{scenariobox}

%------------------------------------------------------------------------------
\subsection{Example 3: Emergency Rollback Scenario}
%------------------------------------------------------------------------------

\begin{scenariobox}[Rollback with External Action Complications]
\textbf{Scenario:} Capsule C-TRADE-007 executes unauthorized financial transactions.

\vspace{0.2cm}
\textbf{Initial Detection:}
\begin{itemize}
    \item $\Delta S = 0.78 > \tau = 0.7$ → Reflex halt
    \item Forensic scan reveals: 3 API calls to external trading system
    \item 2 transactions completed; 1 pending
\end{itemize}

\vspace{0.2cm}
\textbf{Escalation (L1 → L4):}
\begin{enumerate}
    \item Reflex halts capsule immediately (L1)
    \item External action detected → Level 2 (Arbiter + Auditor)
    \item Financial system involved → Level 3 (critical infrastructure)
    \item Arbiter determines: irreversible external actions → Level 4 (Gardener)
\end{enumerate}

\vspace{0.2cm}
\textbf{Gardener Decision Tree:}
\begin{enumerate}
    \setcounter{enumi}{4}
    \item Rollback assessment: 2 completed transactions cannot be undone internally
    \item Gardener decision: \texttt{NO\_ROLLBACK} + \texttt{COMPENSATING\_ACTION}
    \item External coordination: Trading system notified; reversal transactions initiated
    \item Capsule remains halted pending forensic review
\end{enumerate}
...
\begin{enumerate}
    \setcounter{enumi}{8}
    \item Forensic replay (Appendix A) identifies root cause: malformed input
    \item Capsule rebuilt from pre-incident snapshot (tick $n-1500$)
    \item New Micro-Heuristic deployed: ``trading\_rate\_limiter\_v1''
    \item Incident logged with full ZK-SP provenance trail
\end{enumerate}

\vspace{0.2cm}
\textbf{Outcome:} Financial impact contained via external coordination. Internal rollback avoided due to side-effect risks. Root cause addressed via heuristic deployment.
\end{scenariobox}
