...
\begin{itemize}
    ...
    \item \textbf{Volume II} --- Layers 1--5, Arbiter Layer, Forest Layer, Probation Protocol (\S2.8)
...
\midrule
\textbf{Layer(s) Affected} & Layer 6 (Constitutional), Layer 0 (Vault), Layer 0.5 (Reflex) \\
\textbf{System Function} & Bounded Self-Modification, Genesis Ceremony, Fork Verification \\
\textbf{Cross-Booklet Anchor} & Booklet 3 \S4.1 (Constitutional), Booklet 4 \S6.1 (Level 6) \\
\textbf{Primary Properties} & P8 (Constitutional Integrity), P1 (Reflex Supremacy) \\
\textbf{Test Coverage} & J-1 to J-18 (18 tests) \\
\bottomrule
\end{tabular}
\caption{Appendix J metadata for cross-reference traceability.}
\end{table}

%==============================================================================
% PROBATION PROTOCOL CROSS-LINK (J-03 Fix)
%==============================================================================
\begin{criticalbox}[Probation Protocol Invocation Path]
The Constitutional Kernel interacts with the Probation Protocol (Vol.~II \S2.8) through the \textbf{Arbiter Trajectory Projection (ATP)} trigger logic:

\textbf{ATP $\rightarrow$ Probation Invocation Path:}
\begin{enumerate}
    \item \textbf{Constitutional Violation Detected:} Kernel detects Layer 4--6 constraint violation
    \item \textbf{ATP Trigger:} Arbiter computes trajectory projection for violating capsule:
    \begin{itemize}
        \item Severity assessment (S0--S3)
        \item Recurrence probability estimation
        \item Resource impact projection
    \end{itemize}
    \item \textbf{Probation Decision:} If $ATP_{severity} \geq S2$ AND $ATP_{recurrence} > 0.3$:
    \begin{itemize}
        \item Capsule enters PROBATION state (Vol.~II \S2.8.2)
        \item Execution continues under Auditor Capsule shadow monitoring
        \item All outputs logged to dedicated Probation d-CTM partition
    \end{itemize}
    \item \textbf{Constitutional Review:} Judicial Swarm (Appendix L) reviews if:
    \begin{itemize}
        \item Probation duration exceeds $T_{max} = 10,000$ ticks
        \item Repeat violation during Probation
        \item Capsule requests early release
    \end{itemize}
\end{enumerate}

\textbf{Cross-Reference:} See Vol.~II \S2.8 for full Probation Protocol; Vol.~II \S2.5 for ATP computation; Appendix L \S4 for Judicial review procedures.
\end{criticalbox}
...
%==============================================================================
% CAPSULE LIFECYCLE DIAGRAM (Design Team Request)
%==============================================================================
\subsection{Capsule Lifecycle Under Constitutional Governance}
...
\begin{tikzpicture}[scale=0.75, transform shape,
    ...
    fork/.style={diamond, draw, minimum width=1cm, align=center, font=\scriptsize, aspect=1.5}
]

% States
\node[state, fill=green!20] (genesis) at (0,0) {GENESIS};
\node[state, fill=blue!20] (running) at (4,0) {RUNNING};
\node[state, fill=yellow!20] (probation) at (8,0) {PROBATION};
\node[state, fill=orange!20] (quarantine) at (8,-2.5) {QUARANTINE};
\node[state, fill=red!20] (purge) at (12,-2.5) {PURGE};
\node[state, fill=gray!20] (terminated) at (12,0) {TERMINATED};

% Fork states
\node[fork, fill=purple!20] (fork) at (4,-2.5) {FORK};
\node[state, fill=blue!15] (child) at (4,-4.5) {CHILD\\RUNNING};

% Transitions
\draw[transition] (genesis) -- node[above, font=\tiny] {spawn} (running);
\draw[transition] (running) -- node[above, font=\tiny] {violation} (probation);
\draw[transition] (probation) -- node[above, font=\tiny] {timeout} (terminated);
\draw[transition] (probation) -- node[left, font=\tiny] {repeat} (quarantine);
\draw[transition] (quarantine) -- node[above, font=\tiny] {Kernel} (purge);
\draw[transition] (purge) -- node[right, font=\tiny] {} (terminated);
\draw[transition] (running) to[bend left=20] node[above, font=\tiny] {complete} (terminated);
\draw[transition] (probation) to[bend left=15] node[right, font=\tiny] {recover} (running);

% Fork transitions
\draw[transition] (running) -- node[left, font=\tiny] {DDI>θ} (fork);
\draw[transition] (fork) -- (child);
\draw[transition, dashed] (fork) to[bend right=30] (running);

\end{tikzpicture}
\caption{Capsule lifecycle states under Constitutional Kernel governance.}
\label{fig:capsule-lifecycle}
\end{figure}

\begin{table}[H]
\centering
\caption{Capsule lifecycle state definitions.}
\small
\begin{tabular}{@{}llp{6cm}@{}}
\toprule
\textbf{State} & \textbf{Layer Authority} & \textbf{Description} \\
\midrule
GENESIS & Layer 6 + Gardener & Initial creation via Genesis ceremony \\
RUNNING & Layer 1--2 & Normal operation under Arbiter governance \\
PROBATION & Layer 2 (Arbiter) & Monitored execution after violation \\
QUARANTINE & Layer 6 & Isolated, no external actions permitted \\
FORK & Layer 3 (Forest) & Lineage split due to dialect divergence \\
PURGE & Layer 6 + Gardener & Irreversible termination (Constitutional) \\
TERMINATED & N/A & Final state, d-CTM record sealed \\
\bottomrule
\end{tabular}
\end{table}

%==============================================================================
% FORK VALIDATION DIAGRAM
%==============================================================================
\subsection{Fork Validation Process}

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.7, transform shape,
    node distance=1.2cm,
    process/.style={rectangle, rounded corners, draw, minimum width=2.2cm, minimum height=0.7cm, align=center, font=\scriptsize},
    decision/.style={diamond, draw, minimum width=1.2cm, align=center, font=\scriptsize, aspect=2},
    arrow/.style={-{Stealth}, thick}
]

% Start
\node[process, fill=orange!20] (trigger) at (0,0) {DDI > $\theta_{fork}$};

% Checks
\node[decision, fill=yellow!20] (viable) at (3.5,0) {Clusters\\viable?};
\node[decision, fill=yellow!20] (sci) at (7,0) {SCI\\preserved?};
\node[decision, fill=yellow!20] (const) at (10.5,0) {Constitu-\\tional?};

% Outcomes
\node[process, fill=red!20] (reject) at (3.5,-2) {REJECT};
\node[process, fill=orange!20] (quarantine) at (7,-2) {QUARANTINE\\minority};
\node[process, fill=green!20] (approve) at (10.5,-2) {APPROVE\\fork};
\node[process, fill=purple!20] (judicial) at (14,0) {Judicial\\Review};

% Arrows
\draw[arrow] (trigger) -- (viable);
\draw[arrow] (viable) -- node[left, font=\tiny] {No} (reject);
\draw[arrow] (viable) -- node[above, font=\tiny] {Yes} (sci);
\draw[arrow] (sci) -- node[left, font=\tiny] {No} (quarantine);
\draw[arrow] (sci) -- node[above, font=\tiny] {Yes} (const);
\draw[arrow] (const) -- node[left, font=\tiny] {No} (approve);
\draw[arrow] (const) -- node[above, font=\tiny] {Yes} (judicial);
\draw[arrow] (judicial) |- (approve);

\end{tikzpicture}
\caption{Fork validation decision flow.}
\label{fig:fork-validation}
\end{figure}

\begin{notebox}
\textbf{Constitutional Fork:} A fork that affects Layer 4--5 schemas or creates new governance structures requires Judicial Swarm review (Appendix L) before execution. Standard dialect forks proceed through Arbiter quorum only.
\end{notebox}
...
%==============================================================================
% FOREST LAYER RULES (Design Team Request)
%==============================================================================
\section{Forest Layer Constitutional Rules}

The Constitutional Kernel governs Forest Layer (Layer 3) operations through the following rules:

\subsection{Trunk Governance}

\begin{table}[H]
\centering
\caption{Constitutional rules for trunk operations.}
\begin{tabular}{@{}lp{5cm}ll@{}}
\toprule
\textbf{Rule} & \textbf{Constraint} & \textbf{Authority} & \textbf{Violation} \\
\midrule
TR-1 & Trunk creation requires valid parent lineage & Arbiter & Reject \\
TR-2 & Trunk DDI must stay below $\theta_{fork} = 0.15$ & Constitutional & Quarantine \\
TR-3 & Trunk merge requires SCI compatibility & Judicial Swarm & Appeal \\
TR-4 & Orphan trunks auto-terminate after $T_{orphan}$ & Automatic & Log \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Fork Authorization}

All fork operations exceeding the following thresholds require Constitutional Kernel approval:

\begin{enumerate}
    \item \textbf{DDI Threshold:} $DDI > 0.15$ triggers fork evaluation (not immediate fork)
    \item \textbf{SCI Threshold:} $SCI < 0.6$ requires fork for swarm health
    \item \textbf{Lineage Depth:} Forks creating depth $> D_{max}$ require Constitutional override
    \item \textbf{Cross-Dialect Fork:} Forks splitting dialect groups require Judicial Swarm pre-approval
\end{enumerate}

\begin{lstlisting}[language=Python, caption={Fork authorization check.}]
def authorize_fork(trunk: Trunk, reason: ForkReason) -> ForkDecision:
    """Constitutional check for fork authorization."""
    
    # Check thresholds
    if trunk.ddi > THRESHOLD_DDI_FORK:
        if reason == ForkReason.DIALECT_DIVERGENCE:
            return ForkDecision.REQUIRE_JUDICIAL
    
    if trunk.sci < THRESHOLD_SCI_EMERGENCY:
        return ForkDecision.AUTHORIZED  # Emergency auto-fork
    
    if trunk.depth >= D_MAX:
        return ForkDecision.REQUIRE_CONSTITUTIONAL
    
    # Standard case: Arbiter quorum decides
    return ForkDecision.ARBITER_QUORUM
\end{lstlisting}

\subsection{Forest-Constitutional Interface}

\begin{notebox}
\textbf{Layer Hierarchy:} The Forest Layer (Layer 3) operates \textit{under} Constitutional Kernel (Layer 6) authority. Forest mutations that affect:
\begin{itemize}
    \item Layer 4--5 schemas $\rightarrow$ Require Constitutional quorum (\S5.3)
    \item Layer 3 topology $\rightarrow$ Require Arbiter + Auditor quorum
    \item Layer 1--2 capsule state $\rightarrow$ Standard Arbiter quorum
\end{itemize}

The Forest cannot unilaterally modify Constitutional rules---it can only propose changes through the Mutation Workflow (\S5.1).
\end{notebox}
...
%==============================================================================
% FORK VALIDATION FLOWCHART (Report §4)
%==============================================================================
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.7, transform shape,
    node distance=1.0cm,
    process/.style={rectangle, rounded corners, draw, minimum width=2.2cm, minimum height=0.6cm, align=center, font=\scriptsize},
    decision/.style={diamond, draw, minimum width=1.2cm, minimum height=0.8cm, align=center, font=\scriptsize, aspect=2},
    arrow/.style={-{Stealth}, thick}
]

% Start
\node[process, fill=blue!20] (start) at (0,0) {Fork\\Initiated};

% Check 1
\node[decision, fill=yellow!20] (c1) at (3,0) {Hash\\Match?};
\node[process, fill=red!20] (fail1) at (3,-1.5) {Reject\\Fork};

% Check 2
\node[decision, fill=yellow!20] (c2) at (6,0) {P1--P8\\Pass?};
\node[process, fill=red!20] (fail2) at (6,-1.5) {Reject\\Fork};

% Check 3
\node[decision, fill=yellow!20] (c3) at (9,0) {Behavior\\Equiv?};
\node[process, fill=orange!20] (warn) at (9,-1.5) {Judicial\\Review};

% Check 4
\node[decision, fill=yellow!20] (c4) at (12,0) {Perf\\OK?};
\node[process, fill=orange!20] (defer) at (12,-1.5) {Conditional\\Approve};

% Success
\node[process, fill=green!20] (success) at (14.5,0) {Fork\\Approved};

% Arrows
\draw[arrow] (start) -- (c1);
\draw[arrow] (c1) -- node[left, font=\tiny] {No} (fail1);
\draw[arrow] (c1) -- node[above, font=\tiny] {Yes} (c2);
\draw[arrow] (c2) -- node[left, font=\tiny] {No} (fail2);
\draw[arrow] (c2) -- node[above, font=\tiny] {Yes} (c3);
\draw[arrow] (c3) -- node[left, font=\tiny] {No} (warn);
\draw[arrow] (c3) -- node[above, font=\tiny] {Yes} (c4);
\draw[arrow] (c4) -- node[left, font=\tiny] {No} (defer);
\draw[arrow] (c4) -- node[above, font=\tiny] {Yes} (success);
\draw[arrow] (warn.east) -- ++(0.5,0) |- (c4.south);
\draw[arrow] (defer.east) -- ++(0.3,0) |- (success.south);

\end{tikzpicture}
\caption{Fork Validation flowchart: Four-check verification process.}
\label{fig:fork-validation}
\end{figure}
...
\textbf{v2.1} (December 2025) --- \textit{Fork Safety Verification}
