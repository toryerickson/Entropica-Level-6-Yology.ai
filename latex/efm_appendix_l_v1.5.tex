...
%==============================================================================
% ARBITRATION FLOWCHART (Design Team Request)
%==============================================================================
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.75, transform shape,
    node distance=1.2cm,
    process/.style={rectangle, rounded corners, draw, minimum width=2.5cm, minimum height=0.7cm, align=center, font=\scriptsize},
    decision/.style={diamond, draw, minimum width=1.5cm, minimum height=1cm, align=center, font=\scriptsize, aspect=2},
    arrow/.style={-{Stealth}, thick}
]

% Start
\node[process, fill=blue!20] (start) at (0,0) {Dispute\\Detected};

% Level 1
\node[decision, fill=yellow!20] (d1) at (3,0) {Single\\Arbiter?};
\node[process, fill=green!20] (local) at (3,-2) {Local\\Resolution};

% Level 2
\node[decision, fill=yellow!20] (d2) at (6,0) {Cross-\\Dialect?};
\node[process, fill=orange!20] (swarm) at (6,-2) {Judicial\\Swarm};

% Level 3
\node[decision, fill=yellow!20] (d3) at (9,0) {Consti-\\tutional?};
\node[process, fill=red!20] (kernel) at (9,-2) {Layer 6\\Review};

% End states
\node[process, fill=gray!20] (resolved) at (6,-4) {Precedent\\Logged};

% Arrows
\draw[arrow] (start) -- (d1);
\draw[arrow] (d1) -- node[left, font=\tiny] {Yes} (local);
\draw[arrow] (d1) -- node[above, font=\tiny] {No} (d2);
\draw[arrow] (d2) -- node[left, font=\tiny] {Yes} (swarm);
\draw[arrow] (d2) -- node[above, font=\tiny] {No} (d3);
\draw[arrow] (d3) -- node[left, font=\tiny] {Yes} (kernel);
\draw[arrow] (d3.east) -- ++(1,0) |- node[right, font=\tiny, pos=0.25] {No} (swarm);
\draw[arrow] (local) -- (resolved);
\draw[arrow] (swarm) -- (resolved);
\draw[arrow] (kernel) |- (resolved);

\end{tikzpicture}
\caption{Arbitration escalation flowchart: Dispute $\rightarrow$ Resolution path.}
\label{fig:arbitration-flow}
\end{figure}
