...
\subsection{Test Rationale: $\tau_{spawn}$ Parameter}

The $\tau_{spawn}$ (spawn threshold) parameter is the most critical tunable in ASG because it directly gates capsule creation. The following rationale explains why specific tests exist:

\begin{table}[H]
\centering
\small
\caption{$\tau_{spawn}$ test rationale.}
\begin{tabular}{@{}p{2.5cm}p{4cm}p{6cm}@{}}
\toprule
\textbf{Test} & \textbf{$\tau$ Behavior} & \textbf{Why This Test Exists} \\
\midrule
Health degradation (T1) & $\tau \uparrow 0.05$ when LOW & Prevents spawn cascade during stress. Without this, unhealthy swarms spawn more unhealthy capsules, creating feedback death spiral. \\
\addlinespace
Bounded adaptation (T2) & $\tau \in [\tau_{min}, \tau_{max}]$ & Prevents adversarial manipulation. A compromised ASG cannot lower $\tau$ to 0 (spawn freely) or raise to 1 (spawn lockout). \\
\addlinespace
Resource pressure (T6) & $\tau \uparrow$ when vault full & Vault resource exhaustion is an early warning. Raising $\tau$ preemptively prevents OOM-style cascade failures. \\
\addlinespace
Healthy + low spawn (T5) & $\tau \downarrow 0.02$ slowly & Recovery must be asymmetric: fast tightening (crisis response) but slow relaxation (prevent oscillation). The 0.05/0.02 ratio is empirically derived from swarm simulations. \\
\bottomrule
\end{tabular}
\end{table}

\begin{notebox}
\textbf{Why 0.05 increase vs. 0.02 decrease?} This asymmetry implements \textit{defensive adaptation}: the system tightens quickly during stress (respond to threats) but relaxes slowly during recovery (avoid premature relaxation). This pattern is derived from control theory's ``slow integral, fast derivative'' principle for stable feedback systems.
\end{notebox}
...
\bottomrule
\end{tabular}
\caption{ASG test suite (v1.3) --- 23 tests covering all gaps.}
\end{table}
...
\textbf{v1.3} (December 2025) --- \textit{Complete Gap Coverage}
\begin{itemize}[noitemsep]
