...
\midrule
\textbf{Layer(s) Affected} & Layer 0.5 (Reflex), Layer 1 (Execution), Layer 2 (Arbiter) \\
\textbf{System Function} & Forensic State Serialization, Audit Trail, Rollback \\
\textbf{Cross-Booklet Anchor} & Booklet 1 \S3.4, Booklet 2 \S4.1, Booklet 4 \S2.3 \\
\textbf{Primary Properties} & P4 (Audit Completeness), P5 (Lineage Accountability) \\
\textbf{Test Coverage} & A-1 to A-6 (6 tests) \\
\bottomrule
\end{tabular}
\caption{Appendix A metadata for cross-reference traceability.}
\end{table}

%==============================================================================
% SEMANTIC DISCOVERY SYMBOLS (A-01 Fix)
%==============================================================================
\begin{table}[H]
\centering
\caption{Semantic discovery symbol tags for forensic artifacts.}
\label{tab:symbol-tags}
\begin{tabular}{@{}llp{6cm}@{}}
\toprule
\textbf{Symbol Tag} & \textbf{Category} & \textbf{Description} \\
\midrule
\texttt{FSS:TRACE} & Control Loop & Standard control loop trace record \\
\texttt{FSS:SNAP} & Snapshot & Full capsule state snapshot \\
\texttt{FSS:DELTA} & Entropy & Behavioral entropy delta record \\
\texttt{FSS:ESCAL} & Escalation & Escalation event with severity \\
\texttt{FSS:ROLLBACK} & Recovery & Rollback initiation marker \\
\texttt{FSS:RESTORE} & Recovery & State restoration confirmation \\
\texttt{FSS:ANCHOR} & ZK-SP & Cryptographic anchor point \\
\texttt{FSS:PROBATION} & Governance & Probation entry/exit marker \\
\texttt{FSS:GARDENER} & Override & Human override intervention \\
\texttt{FSS:GHOST} & Archaeology & Ghost capsule detection marker \\
\bottomrule
\end{tabular}
\end{table}
...
%==============================================================================
% LAYER 6 CONSTITUTIONAL ROLLBACK CONSTRAINTS (Report §3)
%==============================================================================
\subsection{Layer 6 Constitutional Rollback Constraints}

The Constitutional Kernel (Appendix J) imposes additional constraints on forensic rollback operations:

\begin{table}[H]
\centering
\caption{Constitutional Kernel limits on rollback operations.}
\begin{tabular}{@{}p{3cm}p{4cm}p{5cm}@{}}
\toprule
\textbf{Constraint} & \textbf{Limit} & \textbf{Rationale} \\
\midrule
Rollback Depth & $\leq 10,000$ ticks & Prevents temporal instability \\
Cross-Trunk Rollback & Requires Judicial quorum & Forest integrity protection \\
Constitutional State & Never rollbackable & Layer 6 immutability \\
Genesis State & Never rollbackable & Root identity preservation \\
Post-Fork State & Requires Fork Verification & Lineage consistency \\
\bottomrule
\end{tabular}
\end{table}

\begin{criticalbox}[Layer 6 Rollback Authorization]
Rollback operations affecting Layer 4--6 schemas require Constitutional Kernel authorization:

\begin{lstlisting}[language=Python, caption={Layer 6 rollback authorization check.}]
def authorize_rollback(snapshot: ForensicSnapshot, 
                       target_tick: int) -> RollbackDecision:
    """Check Constitutional Kernel constraints on rollback."""
    
    # Check rollback depth limit
    current_tick = get_current_tick()
    if (current_tick - target_tick) > MAX_ROLLBACK_DEPTH:
        return RollbackDecision.REJECT_DEPTH_EXCEEDED
    
    # Check if target state includes Constitutional changes
    if snapshot.contains_layer_6_changes():
        return RollbackDecision.REJECT_CONSTITUTIONAL
    
    # Check if rollback crosses fork boundary
    if snapshot.crosses_fork_boundary():
        if not judicial_swarm_approved(snapshot):
            return RollbackDecision.REQUIRE_JUDICIAL
    
    # Check Genesis protection
    if snapshot.is_genesis_state():
        return RollbackDecision.REJECT_GENESIS_PROTECTED
    
    return RollbackDecision.AUTHORIZED
\end{lstlisting}

\textbf{Cross-Reference:} See Appendix J \S7 (Emergency Constitutional Override) for rollback during catastrophic events.
\end{criticalbox}
